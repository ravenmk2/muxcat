...
func newRequestCmd() *cobra.Command {
	c := &cobra.Command{
		...
		Short: "Raw request passthrough to the Jenkins server",
		Long: `Raw request passthrough to the Jenkins server (curl semantics).

The path is appended to the instance base URL verbatim and must start
with /, e.g.:
  muxcat jk request GET /api/json
  muxcat jk request GET /job/my-job/api/json
...
GET/HEAD are allowed.`,
  ...
  muxcat jenkins request GET "/job/my-job/lastBuild/api/json?tree=result"
  muxcat jenkins request POST /job/my-job/build --file params.json
  cat body.json | muxcat jk request POST /job/my-job/doDelete --file -`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !strings.HasPrefix(path, "/") {
				return output.NewError(output.CodeConfigInvalid,
					"path must start with /: "+path, "example: /api/json")
			}
