...
// FileName is the jenkins connector's config file name.
const FileName = "jenkins.json"

// Instance is a Jenkins endpoint: the base URL only. Credentials live on
// connections, so one instance can serve multiple connections (different
// users).
...
// usage policies. Token is the Jenkins API token (not the web login
// password), stored as an enc:v1: blob and never echoed back in output.
...
// Config is the jenkins.json model.
type Config struct {
...
// loadConfig reads jenkins.json; a missing file yields an empty config.
...
func resolve(cfg *Config, flagConn string) (string, Connection, error) {
	...
	if name == "" {
		return "", Connection{}, output.NewError(output.CodeConnNotFound,
			...
			"specify one with -c/--conn, or create one with muxcat jenkins conn add <name> --url <url> --set-default")
	}
...
// auth is built from the connection's username/token, and plaintext
// credentials never touch the config file.
func normalizeURL(raw string) (string, error) {
	u, err := url.Parse(strings.TrimSpace(raw))
	if err != nil || u.Host == "" || (u.Scheme != "http" && u.Scheme != "https") {
		return "", output.NewError(output.CodeConfigInvalid,
			"invalid url: "+raw, "example: http://127.0.0.1:8080 or https://jenkins.example.com")
	}
	if u.User != nil {
		return "", output.NewError(output.CodeConfigInvalid,
			"url must not contain credentials (user:password@host)",
			"pass credentials with --username/--token; they are stored encrypted")
	}
